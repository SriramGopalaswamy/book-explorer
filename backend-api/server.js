...
client.connect()
  .then(() => {
    ...
    // Set default schema - include auth schema for authentication
    return client.query(`SET search_path TO ${SCHEMA_NAME}, auth, public;`);
  })
  .then(() => {
    console.log(`✅ Using schema: ${SCHEMA_NAME} (with auth)`);
  })
  .catch(err => {
...
async function syncProfileFromMS365(userId, fullName, jobTitle, department, phone, email, managerEmail) {
  try {
    ...
    let managerId = null;
    if (managerEmail) {
      const { rows: managerProfiles } = await client.query(
