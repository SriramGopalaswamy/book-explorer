...
-- The entry-protection trigger has a DELETE bug (RETURN NEW = NULL for
-- DELETE → silently cancels cascade), so we disable it around the DELETE.
...
-- Expected: DELETE 1 (cascades to 35 payroll_entries rows)
