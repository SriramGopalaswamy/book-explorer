...
BEGIN
  ...
  _checks := _checks || jsonb_build_object('id','RC-012','category','Cross-Tenant Isolation','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Cross-org leaves: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-013','category','Cross-Tenant Isolation','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Cross-org attendance: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-014','category','Cross-Tenant Isolation','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Cross-org payroll: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  SELECT count(*) INTO _count FROM invoices WHERE status NOT IN ('draft','sent','partially_paid','paid','overdue','cancelled','void') AND is_deleted=false;
  _checks := _checks || jsonb_build_object('id','RC-015','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Invalid inv status: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- RC-016: Added 'approved' as valid bill status
  SELECT count(*) INTO _count FROM bills WHERE status NOT IN ('draft','received','approved','partially_paid','paid','overdue','cancelled','void') AND is_deleted=false;
  _checks := _checks || jsonb_build_object('id','RC-016','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Invalid bill status: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  SELECT count(*) INTO _count FROM leave_requests WHERE status NOT IN ('pending','approved','rejected','cancelled');
  _checks := _checks || jsonb_build_object('id','RC-017','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Invalid leave status: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- RC-018: Added 'locked','processed' as valid payroll run statuses
  SELECT count(*) INTO _count FROM payroll_runs WHERE status NOT IN ('draft','processing','computed','approved','completed','cancelled','failed','locked','processed');
  _checks := _checks || jsonb_build_object('id','RC-018','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Invalid payroll status: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  SELECT count(*) INTO _count FROM payroll_runs WHERE status IN ('processing','draft') AND created_at < now()-interval '48 hours';
  _checks := _checks || jsonb_build_object('id','RC-019','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Stale payroll runs: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-020','category','Duplicate Detection','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Dup invoices: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-021','category','Duplicate Detection','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Dup bills: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- RC-022: Exclude superseded payroll records from dup check
  SELECT count(*) INTO _count FROM (SELECT user_id, pay_period, organization_id FROM payroll_records WHERE status != 'superseded' GROUP BY user_id, pay_period, organization_id HAVING count(*)>1) d;
  _checks := _checks || jsonb_build_object('id','RC-022','category','Duplicate Detection','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Dup payroll: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  SELECT count(*) INTO _count FROM (SELECT user_id, date FROM attendance_records GROUP BY user_id, date HAVING count(*)>1) d;
  _checks := _checks || jsonb_build_object('id','RC-023','category','Duplicate Detection','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Dup attendance: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  BEGIN SELECT count(*) INTO _count FROM (SELECT employee_code, organization_id FROM profiles WHERE employee_code IS NOT NULL GROUP BY employee_code, organization_id HAVING count(*)>1) d; EXCEPTION WHEN undefined_column THEN _count := 0; END;
  _checks := _checks || jsonb_build_object('id','RC-024','category','Duplicate Detection','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Dup emp codes: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  SELECT count(*) INTO _count FROM pg_policies WHERE tablename='profiles' AND schemaname='public';
  _checks := _checks || jsonb_build_object('id','RC-025','category','Trigger & Schema Safety','severity',CASE WHEN _count=0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count>0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Profile RLS policies: %s',_count),'auto_fix_possible',false,'affected_count',CASE WHEN _count=0 THEN 1 ELSE 0 END);

  SELECT count(*) INTO _count FROM pg_policies WHERE tablename='financial_records' AND schemaname='public';
  _checks := _checks || jsonb_build_object('id','RC-026','category','Trigger & Schema Safety','severity',CASE WHEN _count=0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count>0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Financial RLS policies: %s',_count),'auto_fix_possible',false,'affected_count',CASE WHEN _count=0 THEN 1 ELSE 0 END);

  SELECT count(*) INTO _count FROM pg_policies WHERE tablename='payroll_records' AND schemaname='public';
  _checks := _checks || jsonb_build_object('id','RC-027','category','Trigger & Schema Safety','severity',CASE WHEN _count=0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count>0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Payroll RLS policies: %s',_count),'auto_fix_possible',false,'affected_count',CASE WHEN _count=0 THEN 1 ELSE 0 END);

  SELECT count(*) INTO _count FROM information_schema.tables t LEFT JOIN (SELECT DISTINCT schemaname, tablename FROM pg_policies) p ON p.schemaname=t.table_schema AND p.tablename=t.table_name WHERE t.table_schema='public' AND t.table_type='BASE TABLE' AND p.tablename IS NULL AND t.table_name NOT IN ('schema_migrations','spatial_ref_sys');
  _checks := _checks || jsonb_build_object('id','RC-028','category','Trigger & Schema Safety','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('No RLS tables: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-029','category','Trigger & Schema Safety','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Disabled triggers: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-030','category','Trigger & Schema Safety','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('RLS disabled tables: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-031','category','Data Lifecycle','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('No deleted_at: %s',_count),'auto_fix_possible',true,'affected_count',_count);

  SELECT count(*) INTO _count FROM invoices WHERE is_deleted=true AND deleted_at < now()-interval '180 days';
  SELECT count(*) INTO _count2 FROM bills WHERE is_deleted=true AND deleted_at < now()-interval '180 days';
  _count := _count+_count2;
  _checks := _checks || jsonb_build_object('id','RC-032','category','Data Lifecycle','severity','LOW','status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Old deleted (6mo+): %s',_count),'auto_fix_possible',true,'affected_count',_count);

  SELECT count(*) INTO _count FROM invoices WHERE amount=0 AND status NOT IN ('draft','cancelled','void') AND is_deleted=false;
  _checks := _checks || jsonb_build_object('id','RC-033','category','Default Value Traps','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Zero amt invoices: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  -- RC-036: Only check POSTED journal entries for balance
  SELECT count(*) INTO _count FROM (SELECT jl.journal_entry_id FROM journal_lines jl JOIN journal_entries je ON je.id=jl.journal_entry_id WHERE je.status='posted' GROUP BY jl.journal_entry_id HAVING ABS(SUM(jl.debit)-SUM(jl.credit))>0.01) d;
  ...
  SELECT count(*) INTO _count FROM (SELECT account_code, organization_id FROM chart_of_accounts GROUP BY account_code, organization_id HAVING count(*)>1) d;
  ...
  BEGIN SELECT count(*) INTO _count FROM financial_records fr JOIN fiscal_periods fp ON fr.organization_id=fp.organization_id AND fr.record_date BETWEEN fp.start_date AND fp.end_date WHERE fp.status='locked' AND fr.updated_at>fp.closed_at; EXCEPTION WHEN undefined_column THEN _count := 0; END;
  ...
  SELECT count(*) INTO _count FROM leave_requests WHERE from_date>to_date;
  ...
  BEGIN SELECT count(*) INTO _count FROM organizations o WHERE o.status='active' AND NOT EXISTS (SELECT 1 FROM fiscal_years fy WHERE fy.organization_id=o.id); EXCEPTION WHEN undefined_table THEN _count := 0; END;
  ...
  -- RC-050: Fixed to properly check admin via profiles org scope
  SELECT count(*) INTO _count FROM organizations o WHERE o.status='active' AND NOT EXISTS (
    SELECT 1 FROM user_roles ur WHERE ur.role='admin' AND EXISTS (
      SELECT 1 FROM profiles p WHERE p.user_id=ur.user_id AND p.organization_id=o.id
    )
  );
  ...
  BEGIN SELECT count(*) INTO _count FROM stock_adjustment_items WHERE difference_qty=0; EXCEPTION WHEN undefined_table OR undefined_column THEN _count := 0; END;
  ...
  RETURN jsonb_build_object('engine','Root Cause Audit Engine v4.4','run_at',now(),'org_filter',p_org_id,'total_checks',jsonb_array_length(_checks),'checks',_checks);
END;
$$