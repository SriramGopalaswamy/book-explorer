...
DECLARE
  ...
  _rpc_ok boolean;
BEGIN
  -- RC-001 to RC-010: Reference Integrity
  ...
  -- RC-011 to RC-014: Cross-Tenant Isolation
  ...
  -- RC-015 to RC-019: Status Machine Violations
  ...
  SELECT count(*) INTO _count FROM bills WHERE status NOT IN ('draft','received','partially_paid','paid','overdue','cancelled','void') AND is_deleted=false;
  ...
  SELECT count(*) INTO _count FROM payroll_runs WHERE status NOT IN ('draft','processing','computed','approved','completed','cancelled','failed');
  ...
  -- RC-020 to RC-024: Duplicate Detection
  ...
  SELECT count(*) INTO _count FROM (SELECT user_id, pay_period, organization_id FROM payroll_records GROUP BY user_id, pay_period, organization_id HAVING count(*)>1) d;
  ...
  BEGIN
    SELECT count(*) INTO _count FROM (SELECT employee_code, organization_id FROM profiles WHERE employee_code IS NOT NULL GROUP BY employee_code, organization_id HAVING count(*)>1) d;
  EXCEPTION WHEN undefined_column THEN _count := 0;
  END;
  ...
  -- RC-025 to RC-030: Trigger & Schema Safety
  ...
  -- RC-031 to RC-032: Data Lifecycle
  ...
  -- RC-033 to RC-034: Default Value Traps
  ...
  -- RC-035 to RC-042: Financial Health
  ...
  -- RC-043 to RC-045: Timestamp & Ordering
  ...
  -- FIXED: leave_requests uses from_date/to_date not start_date/end_date
  ...
  -- RC-046 to RC-056: Seeding & Org Completeness
  ...
  -- RC-057 to RC-058: RPC & Engine Health
  ...
  -- RC-059 to RC-061: Payroll & HR Sync
  ...
  -- RC-062 to RC-067: Inventory & Supply Chain
  ...
  -- RC-068 to RC-072: Audit Trail Integrity
  _count := 0;
  ...
  RETURN jsonb_build_object('engine','Root Cause Audit Engine v4.3','run_at',now(),'org_filter',p_org_id,'total_checks',jsonb_array_length(_checks),'checks',_checks);
END;
$$;