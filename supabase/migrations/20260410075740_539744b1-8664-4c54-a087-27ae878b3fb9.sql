...
BEGIN
  ...
  SELECT ur.user_id INTO v_admin_uid
  FROM   public.user_roles ur
  WHERE  ur.organization_id = c_org_id AND ur.role = 'admin'
  ...
  FOR emp IN
    SELECT t.email, t.full_name, t.annual_ctc, t.ann_basic, t.ann_hra, t.ann_other, t.ann_epf
    FROM (VALUES
  ...
  LOOP
    ...
    SELECT id INTO v_auth_id FROM auth.users WHERE lower(email) = emp.email;
    ...
    IF v_admin_uid IS NULL THEN v_admin_uid := v_auth_id; END IF;

    SELECT id INTO v_prof_id FROM public.profiles WHERE user_id = v_auth_id;
    ...
    ELSE
      ...
      SET full_name = emp.full_name, email = emp.email,
          organization_id = c_org_id, status = 'active',
          updated_at = now()
      WHERE id = v_prof_id;
    END IF;
