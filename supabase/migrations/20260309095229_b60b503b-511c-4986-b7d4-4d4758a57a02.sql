...
BEGIN
  ...
  -- RC-024: Dup employee codes - profiles may not have employee_code column
  BEGIN
  ...
  _count := _count+_count2;
  _checks := _checks || jsonb_build_object('id','RC-032','category','Data Lifecycle','severity',CASE WHEN _count>0 THEN 'LOW' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Old deleted (6mo+): %s',_count),'auto_fix_possible',true,'affected_count',_count);
