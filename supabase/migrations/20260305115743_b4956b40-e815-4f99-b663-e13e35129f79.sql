-- Upgrade Verification Engine V2 → V3: Add missing checks both engines lacked
...
BEGIN
  ...
  -- NEW: F6: Duplicate Invoice Numbers
  ...
  -- NEW: W4: Stale Processing Workflows (stuck >24h)
  SELECT count(*) INTO _cnt FROM public.payroll_runs
  ...
  -- NEW: W5: Duplicate Payroll Records (same employee, same period, not superseded)
  SELECT count(*) INTO _cnt FROM (
  ...
  -- NEW: UI4: Payroll Record Orphans
  ...
  -- ═══════════════════════════════════════════════
  -- SECTION 6: OPERATIONAL SAFETY (previously O1, O2)
  -- ═══════════════════════════════════════════════
  ...
  -- NEW: O2: Timestamp Integrity (updated_at < created_at)
  SELECT count(*) INTO _cnt FROM public.profiles
  ...
  -- NEW: O3: Broken Triggers (triggers referencing missing columns)
  FOR _tbl IN
  ...
  -- NEW: O4: Duplicate Attendance Records (same user, same date)
  SELECT count(*) INTO _cnt FROM (
