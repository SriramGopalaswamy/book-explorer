...
-- Allow signing-related column updates on invoices even when in terminal state.
-- This exempts Aadhaar eSign workflow updates from the terminal state block.
  ...
  IF TG_OP = 'UPDATE' THEN
    IF OLD.status = ANY(terminal_states) THEN
      ...
      IF TG_TABLE_NAME = 'invoices' AND NEW.status = OLD.status THEN
        -- Check if ONLY signing-related columns changed
        IF (
          ...
          -- Verify no other material columns changed (status stays the same, amounts untouched)
