-- Two integrity triggers on payroll_entries:
-- 1. BEFORE INSERT/UPDATE: verify entry's organization_id matches
--    the parent payroll_run's organization_id (cross-tenant insert guard).
-- 2. AFTER INSERT/UPDATE/DELETE: recalculate and update the parent
--    payroll_run's aggregate totals (total_gross, total_deductions,
--    total_net, employee_count).

-- ─── 1. Cross-tenant org_id integrity ─────────────────────────────────────
...
BEGIN
  ...
  IF v_run_org IS NULL THEN
    RAISE EXCEPTION 'payroll_run % not found', NEW.payroll_run_id;
  END IF;

  IF NEW.organization_id IS DISTINCT FROM v_run_org THEN
    RAISE EXCEPTION
      'payroll_entries.organization_id (%) does not match '
      'payroll_runs.organization_id (%) for run %',
...
-- ─── 2. Recalculate payroll_run aggregate totals ───────────────────────────
