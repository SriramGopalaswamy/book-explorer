...
BEGIN
  ...
  _checks := _checks || jsonb_build_object('id','RC-010','category','Reference Integrity','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Journal lines with orphan account refs: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-014','category','Cross-Tenant Isolation','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Leave requests org mismatch: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  ...
  -- RC-019: FIXED — use pay_period column
  ...
  _checks := _checks || jsonb_build_object('id','RC-020','category','Duplicate Records','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Duplicate bill numbers: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-022','category','Auth-Profile Sync','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Email mismatch profile vs auth: %s',_count),'auto_fix_possible',true,'affected_count',_count);

  SELECT count(*) INTO _count FROM auth.users u WHERE NOT EXISTS (SELECT 1 FROM profiles p WHERE p.user_id=u.id);
  _checks := _checks || jsonb_build_object('id','RC-023','category','Auth-Profile Sync','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Auth users with no profile: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-027','category','RLS & Access Control','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Tables without RLS: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-034','category','Trigger & Schema Safety','severity',CASE WHEN _count>5 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Nullable organization_id columns: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-038','category','Financial Health','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Inactive CoA with recent entries: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-041','category','Financial Health','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Orgs without fiscal periods: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-042','category','Financial Health','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Negative amount records: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  SELECT count(*) INTO _count FROM bills WHERE tds_rate IS NOT NULL AND tds_rate>100;
  _checks := _checks || jsonb_build_object('id','RC-043','category','Financial Health','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('TDS rate > 100%%: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  -- CAT 9: STATUS MACHINE VIOLATIONS
  ...
  _checks := _checks || jsonb_build_object('id','RC-047','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Payroll runs stuck draft 30+ days: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-049','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Work orders invalid status: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-050','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('POs invalid status: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _count := _count+_count2+_count3;
  _checks := _checks || jsonb_build_object('id','RC-051','category','Timestamp Anomalies','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Future created_at: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _count := _count+_count2;
  _checks := _checks || jsonb_build_object('id','RC-052','category','Timestamp Anomalies','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('updated_at < created_at: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-054','category','Seeding & Org Completeness','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Orgs with profiles but no roles: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-055','category','Seeding & Org Completeness','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Orgs with no org_members: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-056','category','Seeding & Org Completeness','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Orgs 5+ employees zero managers: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  -- RC-060: FIXED — match on pay_period instead of FK
  ...
  _checks := _checks || jsonb_build_object('id','RC-060','category','Payroll & HR Sync','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Completed runs with zero records: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-062','category','Inventory & Supply Chain','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Negative stock items: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-063','category','Inventory & Supply Chain','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Stale confirmed POs: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-064','category','Inventory & Supply Chain','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Stale confirmed SOs: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-065','category','Inventory & Supply Chain','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Completed WOs zero consumption: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-066','category','Inventory & Supply Chain','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Active BOMs zero components: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  SELECT count(*) INTO _count FROM stock_adjustments WHERE quantity=0;
  _checks := _checks || jsonb_build_object('id','RC-067','category','Inventory & Supply Chain','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Zero-quantity adjustments: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-068','category','Audit Trail Integrity','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Missing audit entity types: %s/4',_count),'auto_fix_possible',false,'affected_count',_count);

  SELECT count(*) INTO _count FROM audit_logs WHERE actor_id IS NULL;
  _checks := _checks || jsonb_build_object('id','RC-069','category','Audit Trail Integrity','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('NULL actor_id audit logs: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-070','category','Audit Trail Integrity','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Stale audit orgs (7+ days): %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  EXCEPTION WHEN undefined_table THEN
    _count := 0;
  END;
  _checks := _checks || jsonb_build_object('id','RC-071','category','Audit Trail Integrity','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Stale connector syncs: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-072','category','Audit Trail Integrity','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Pending on disabled workflows: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  RETURN jsonb_build_object(
    'engine','Root Cause Audit Engine v4.1',
    'run_at',now(),
    'org_filter',p_org_id,
