...
BEGIN
  ...
  _checks := _checks || jsonb_build_object('id','RC-006','category','Reference Integrity','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Invalid manager_id: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-007','category','Reference Integrity','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Bills orphan vendors: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-008','category','Reference Integrity','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Invoices orphan customers: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  SELECT count(*) INTO _count FROM expenses e WHERE NOT EXISTS (SELECT 1 FROM profiles p WHERE p.user_id=e.user_id);
  _checks := _checks || jsonb_build_object('id','RC-009','category','Reference Integrity','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Expenses orphan profiles: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-010','category','Reference Integrity','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Journal lines orphan accounts: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- CAT 2: CROSS-TENANT ISOLATION
  SELECT count(*) INTO _count FROM (SELECT user_id FROM profiles GROUP BY user_id HAVING count(DISTINCT organization_id)>1) d;
  _checks := _checks || jsonb_build_object('id','RC-011','category','Cross-Tenant Isolation','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Users in multiple orgs: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-012','category','Cross-Tenant Isolation','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Attendance org mismatch: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-013','category','Cross-Tenant Isolation','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Payroll org mismatch: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-014','category','Cross-Tenant Isolation','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Leave org mismatch: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  SELECT count(*) INTO _count FROM goals g JOIN profiles p ON p.user_id=g.user_id WHERE g.organization_id!=p.organization_id;
  _checks := _checks || jsonb_build_object('id','RC-015','category','Cross-Tenant Isolation','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Goals org mismatch: %s',_count),'auto_fix_possible',true,'affected_count',_count);

  -- CAT 3: DUPLICATE RECORDS
  ...
  _checks := _checks || jsonb_build_object('id','RC-016','category','Duplicate Records','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Duplicate profiles: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-017','category','Duplicate Records','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Duplicate roles: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-018','category','Duplicate Records','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Duplicate invoices: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  SELECT count(*) INTO _count FROM (SELECT user_id,pay_period FROM payroll_records GROUP BY user_id,pay_period HAVING count(*)>1) d;
  _checks := _checks || jsonb_build_object('id','RC-019','category','Duplicate Records','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Duplicate payroll: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-020','category','Duplicate Records','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Duplicate bills: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  SELECT count(*) INTO _count FROM (SELECT user_id,date FROM attendance_records GROUP BY user_id,date HAVING count(*)>1) d;
  _checks := _checks || jsonb_build_object('id','RC-021','category','Duplicate Records','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Duplicate attendance: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- CAT 4: AUTH-PROFILE SYNC
  ...
  _checks := _checks || jsonb_build_object('id','RC-022','category','Auth-Profile Sync','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Email mismatch: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  SELECT count(*) INTO _count FROM auth.users u WHERE NOT EXISTS (SELECT 1 FROM profiles p WHERE p.user_id=u.id);
  _checks := _checks || jsonb_build_object('id','RC-023','category','Auth-Profile Sync','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Auth users no profile: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  SELECT count(*) INTO _count FROM profiles WHERE email IS NULL OR email='';
  _checks := _checks || jsonb_build_object('id','RC-024','category','Auth-Profile Sync','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Ghost profiles: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-025','category','Auth-Profile Sync','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Inactive profiles active auth: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- CAT 5: RLS & ACCESS CONTROL
  ...
  _checks := _checks || jsonb_build_object('id','RC-026','category','RLS & Access Control','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('RLS no policies: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-027','category','RLS & Access Control','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('No RLS: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-028','category','RLS & Access Control','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Records in suspended orgs: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- CAT 6: TRIGGER & SCHEMA SAFETY
  ...
  _checks := _checks || jsonb_build_object('id','RC-032','category','Trigger & Schema Safety','severity',CASE WHEN _count=0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count>0 THEN 'PASS' ELSE 'FAIL' END,'detail',CASE WHEN _count>0 THEN 'PO expected_date exists' ELSE 'PO missing expected_date' END,'auto_fix_possible',true,'affected_count',CASE WHEN _count=0 THEN 1 ELSE 0 END);
  ...
  _checks := _checks || jsonb_build_object('id','RC-033','category','Trigger & Schema Safety','severity',CASE WHEN _count=0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count>0 THEN 'PASS' ELSE 'FAIL' END,'detail',CASE WHEN _count>0 THEN 'SO expected_date exists' ELSE 'SO missing expected_date' END,'auto_fix_possible',true,'affected_count',CASE WHEN _count=0 THEN 1 ELSE 0 END);
  ...
  _checks := _checks || jsonb_build_object('id','RC-034','category','Trigger & Schema Safety','severity',CASE WHEN _count>5 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Nullable org_id: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-035','category','Trigger & Schema Safety','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Missing unique idx: %s — %s',_count,rtrim(_missing_cols,', ')),'auto_fix_possible',true,'affected_count',_count);

  -- CAT 7: FINANCIAL HEALTH
  ...
  _checks := _checks || jsonb_build_object('id','RC-036','category','Financial Health','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Unbalanced journals: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-037','category','Financial Health','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Orgs no CoA: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-038','category','Financial Health','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Inactive CoA recent entries: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  -- RC-040 FIXED: use closed_at instead of updated_at
  SELECT count(*) INTO _count FROM financial_records fr JOIN fiscal_periods fp ON fr.organization_id=fp.organization_id AND fr.record_date BETWEEN fp.start_date AND fp.end_date WHERE fp.status='closed' AND fp.closed_at IS NOT NULL AND fr.created_at > fp.closed_at;
  _checks := _checks || jsonb_build_object('id','RC-040','category','Financial Health','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Entries after period close: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-041','category','Financial Health','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Orgs no fiscal periods: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-042','category','Financial Health','severity',CASE WHEN _count>0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Negative amounts: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  SELECT count(*) INTO _count FROM bills WHERE tds_rate IS NOT NULL AND tds_rate>100;
  _checks := _checks || jsonb_build_object('id','RC-043','category','Financial Health','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('TDS >100%%: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- CAT 8: DATA LIFECYCLE
  SELECT count(*) INTO _count FROM invoices WHERE is_deleted=true AND deleted_at IS NULL;
  _checks := _checks || jsonb_build_object('id','RC-044','category','Data Lifecycle','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Deleted invoices no deleted_at: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  SELECT count(*) INTO _count FROM bills WHERE is_deleted=true AND deleted_at IS NULL;
  _checks := _checks || jsonb_build_object('id','RC-045','category','Data Lifecycle','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Deleted bills no deleted_at: %s',_count),'auto_fix_possible',true,'affected_count',_count);

  -- CAT 9: STATUS MACHINE
  ...
  _checks := _checks || jsonb_build_object('id','RC-046','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Invalid invoice status: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  SELECT count(*) INTO _count FROM payroll_runs WHERE status='draft' AND created_at < now()-interval '30 days';
  _checks := _checks || jsonb_build_object('id','RC-047','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Stale draft runs: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  SELECT count(*) INTO _count FROM payroll_runs WHERE status='processing' AND created_at < now()-interval '24 hours';
  _checks := _checks || jsonb_build_object('id','RC-048','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Stuck processing: %s',_count),'auto_fix_possible',true,'affected_count',_count);
  SELECT count(*) INTO _count FROM work_orders WHERE status NOT IN ('draft','planned','in_progress','completed','cancelled','on_hold');
  _checks := _checks || jsonb_build_object('id','RC-049','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Invalid WO status: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-050','category','Status Machine Violations','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Invalid PO status: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- CAT 10: TIMESTAMP ANOMALIES
  ...
  _count := _count+_count2+_count3;
  _checks := _checks || jsonb_build_object('id','RC-051','category','Timestamp Anomalies','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Future timestamps: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _count := _count+_count2;
  _checks := _checks || jsonb_build_object('id','RC-052','category','Timestamp Anomalies','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('updated < created: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- CAT 11: SEEDING & ORG COMPLETENESS
  SELECT count(*) INTO _count FROM organizations o WHERE NOT EXISTS (SELECT 1 FROM profiles p WHERE p.organization_id=o.id);
  _checks := _checks || jsonb_build_object('id','RC-053','category','Seeding & Org Completeness','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Empty orgs: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-054','category','Seeding & Org Completeness','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Orgs no roles: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-055','category','Seeding & Org Completeness','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Orgs no members: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-056','category','Seeding & Org Completeness','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('5+ employees no managers: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- CAT 12: RPC & ENGINE HEALTH
  BEGIN PERFORM run_financial_verification(); _rpc_ok := true; EXCEPTION WHEN OTHERS THEN _rpc_ok := false; END;
  _checks := _checks || jsonb_build_object('id','RC-057','category','RPC & Engine Health','severity',CASE WHEN NOT _rpc_ok THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _rpc_ok THEN 'PASS' ELSE 'FAIL' END,'detail',CASE WHEN _rpc_ok THEN 'financial_verification OK' ELSE 'financial_verification FAILED' END,'auto_fix_possible',false,'affected_count',CASE WHEN _rpc_ok THEN 0 ELSE 1 END);
  ...
  _checks := _checks || jsonb_build_object('id','RC-058','category','RPC & Engine Health','severity',CASE WHEN _count=0 THEN 'CRITICAL' ELSE 'LOW' END,'status',CASE WHEN _count>0 THEN 'PASS' ELSE 'FAIL' END,'detail',CASE WHEN _count>0 THEN 'root_cause_audit exists' ELSE 'root_cause_audit MISSING' END,'auto_fix_possible',false,'affected_count',CASE WHEN _count=0 THEN 1 ELSE 0 END);

  -- CAT 13: PAYROLL & HR SYNC
  ...
  _checks := _checks || jsonb_build_object('id','RC-059','category','Payroll & HR Sync','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Payroll no attendance: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-060','category','Payroll & HR Sync','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('Completed runs no records: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-065','category','Inventory & Supply Chain','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('WOs no consumption: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  _checks := _checks || jsonb_build_object('id','RC-067','category','Inventory & Supply Chain','severity',CASE WHEN _count>0 THEN 'MEDIUM' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'WARNING' END,'detail',format('Zero-qty adjustments: %s',_count),'auto_fix_possible',false,'affected_count',_count);

  -- CAT 15: AUDIT TRAIL INTEGRITY
  _count := 0;
  ...
  _checks := _checks || jsonb_build_object('id','RC-069','category','Audit Trail Integrity','severity',CASE WHEN _count>0 THEN 'HIGH' ELSE 'LOW' END,'status',CASE WHEN _count=0 THEN 'PASS' ELSE 'FAIL' END,'detail',format('NULL actor audit logs: %s',_count),'auto_fix_possible',false,'affected_count',_count);
  ...
  EXCEPTION WHEN undefined_table THEN _count := 0;
  END;
  ...
  RETURN jsonb_build_object('engine','Root Cause Audit Engine v4.1','run_at',now(),'org_filter',p_org_id,'total_checks',jsonb_array_length(_checks),'checks',_checks);
END;
$$;