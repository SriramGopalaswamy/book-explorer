...
BEGIN
  ...
  -- F4: GL Account Referential Integrity (join through journal_entries for org filter)
  SELECT count(*) INTO _cnt FROM public.journal_lines jl
  ...
  WHERE receipt_url IS NULL AND status != 'rejected' AND (_org_id IS NULL OR organization_id = _org_id);
  _results := _results || jsonb_build_object(
  ...
  -- O2: Stale Unposted Entries (financial_records uses is_posted, not status)
