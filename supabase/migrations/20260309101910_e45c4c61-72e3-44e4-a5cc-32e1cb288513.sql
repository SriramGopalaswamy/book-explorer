...
BEGIN
  ...
  -- RC-010: Fixed to use gl_accounts (actual FK target for journal_lines.gl_account_id)
  ...
  -- RC-038: Now same as RC-010 (both use gl_accounts) - keep for backward compat, check CoA refs separately
  SELECT count(*) INTO _count FROM journal_lines jl WHERE NOT EXISTS (SELECT 1 FROM gl_accounts ga WHERE ga.id=jl.gl_account_id);
  ...
  RETURN jsonb_build_object('engine','Root Cause Audit Engine v4.5','run_at',now(),'org_filter',p_org_id,'total_checks',jsonb_array_length(_checks),'checks',_checks);
END;
$$