...
const RC_CATEGORY_META: Record<string, { icon: React.ElementType; order: number; description: string }> = {
  "Reference Integrity": { icon: Link2, order: 0, description: "NULL references, orphan records, deleted-parent cascades across all modules" },
  "Cross-Tenant Isolation": { icon: Lock, order: 1, description: "Data leaking between organizations (attendance, payroll, leaves, goals)" },
  "Duplicate Records": { icon: Layers, order: 2, description: "Profiles, roles, invoices, bills, payroll, attendance duplicates" },
  "Auth-Profile Sync": { icon: UserCheck, order: 3, description: "Email mismatches, auth users without profiles, inactive profiles with active sessions" },
  "RLS & Access Control": { icon: Shield, order: 4, description: "Row-level security gaps, suspended org writes" },
  "Trigger & Schema Safety": { icon: Bug, order: 5, description: "Trigger guards, schema drift, nullable org_ids, missing constraints" },
  "Financial Health": { icon: Scale, order: 6, description: "Journal balancing, CoA integrity, fiscal periods, negative amounts, TDS rates" },
  "Data Lifecycle": { icon: Trash2, order: 7, description: "Soft-deleted records with inconsistent metadata" },
  "Status Machine Violations": { icon: AlertTriangle, order: 8, description: "Invalid statuses on invoices, payroll, work orders, purchase orders" },
  "Timestamp Anomalies": { icon: Clock, order: 9, description: "Future dates, reversed timestamps" },
  "Seeding & Org Completeness": { icon: Users, order: 10, description: "Orgs missing profiles, roles, members, or manager hierarchy" },
  "RPC & Engine Health": { icon: Server, order: 11, description: "Critical database functions and engine self-tests" },
  "Payroll & HR Sync": { icon: Briefcase, order: 12, description: "Payroll-attendance alignment, empty runs, negative leave balances" },
  "Inventory & Supply Chain": { icon: Package, order: 13, description: "Negative stock, stale POs/SOs, empty BOMs, work order consumption gaps" },
  "Audit Trail Integrity": { icon: ClipboardList, order: 14, description: "Audit coverage gaps, stale connectors, disabled workflow approvals" },
};
      ...
      <Card className="mb-6">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            ...
            Root Cause Audit Engine v4 — 72 checks · 15 categories
          </CardTitle>
          <div className="flex items-center gap-3">
            {result && (
              <>
