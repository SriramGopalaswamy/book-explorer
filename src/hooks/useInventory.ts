...
export function useCreateItem() {
  ...
  return useMutation({
    mutationFn: async (item: Record<string, any>) => {
      ...
      const { data, error } = await supabase.from("items").insert({ ...item, organization_id: orgId, created_by: user.id } as any).select().single();
      if (error) throw error;
      return data;
    },
    onSuccess: () => {
...
export function useCreateWarehouse() {
  ...
  return useMutation({
    mutationFn: async (wh: Record<string, any>) => {
      ...
      const { data, error } = await supabase.from("warehouses").insert({ ...wh, organization_id: orgId } as any).select().single();
      if (error) throw error;
      return data;
    },
    onSuccess: () => {
...
export function useCreateStockAdjustment() {
  ...
  return useMutation({
    mutationFn: async (adj: Record<string, any>) => {
      ...
      const { data, error } = await supabase.from("stock_adjustments").insert({ ...adj, organization_id: orgId, created_by: user.id } as any).select().single();
      if (error) throw error;
      return data;
    },
    onSuccess: () => {
...
export function useCreateUOM() {
  ...
  return useMutation({
    mutationFn: async (uom: Record<string, any>) => {
      ...
      const orgId = orgData?.organizationId;
      if (!orgId) throw new Error("No organization found");
      const { data, error } = await supabase.from("units_of_measure").insert({ ...uom, organization_id: orgId } as any).select().single();
      if (error) throw error;
      return data;
    },
    onSuccess: () => {
